...
class ABB:
    ...
    def __cantidadNodos(self, nodo):
        if nodo is None:
            return 0
        else:
            return 1 + self.__cantidadNodos(nodo.getIzquierdo()) + self.__cantidadNodos(nodo.getDerecho())

    def __sucesores(self, nodo):
        if nodo.getIzquierdo():
            self.__preOrdenRecursivo(nodo.getIzquierdo())
        if nodo.getDerecho():
            self.__preOrdenRecursivo(nodo.getDerecho())
...
# Métodos públicos
    ...
    def padreYHermano(self, dato):
        ...
        if nodo is None:
            print("Árbol vacío")
        elif nodo == self.__raiz:
            print("La raíz no tiene padre ni hermano")
        else:
            print(f"Padre: {padre.getDato()}")
            print(f"Hijo: {nodo.getDato()}")
            if nodo == padre.getIzquierdo():
                if padre.getDerecho():
                    print(f"Hermano: {padre.getDerecho().getDato()}")
                else:
                    print(f"Hermano: no tiene")
            if nodo == padre.getDerecho():
                if padre.getIzquierdo():
                    print(f"Hermano: {padre.getIzquierdo().getDato()}")
                else:
                    print(f"Hermano: no tiene")

    def cantidadNodos(self):
        if self.__raiz is not None:
            return self.__cantidadNodos(self.__raiz)
        else:
            print("Árbol vacío")

    def sucesores(self, dato):
        nodo = self.__buscaRecursivo(self.__raiz, None, dato)[0]
        if nodo:
            self.__sucesores(nodo)
        else:
            print("Árbol vacío")
